...
def index(request):
    ...
    context = {"routes": settings.DYNAMIC_API.keys(), "segment": "dynamic_api"}
    ...
    def get(self, request, **kwargs):
        ...
        try:
            if model_id is not None:
                ...
                all_instances = Utils.get_manager(
                    DYNAMIC_API, kwargs.get("model_name")
                ).all()
                instance_serializer = Utils.get_serializer(
                    DYNAMIC_API, kwargs.get("model_name")
                )
                output = []
                for instance in all_instances:
                    output.append(instance_serializer(instance=instance).data)
        except KeyError:
            return Response(
                data={
